...
const PaymentDropdown: React.FC<PaymentDropdownProps> = ({ onPaymentSelect }) => {
  const [isVirtualAccountOpen, setIsVirtualAccountOpen] = useState<boolean>(false);
  ...
  return (
    <div className="p-4 border-gray-300 relative">
      <h5 className="font-nunito font-bold text-[16px] text-black mb-4">
      ...
      <div className="mb-2">
        <button
          className="flex justify-between items-center py-2 px-4 w-full bg-white border rounded-md"
            ...
            {['BCA', 'Mandiri', 'BNI', 'BRI'].map((bank) => (
              ...
              >
                ...
                <input
                  ...
                  onChange={() => handlePaymentSelect(`${bank} Bank Transfer`)}
                />
              </div>
            ))}
          </div>
      ...
      {/* QRIS Option */}
      <div className="flex justify-between items-center py-2 px-4 bg-white border rounded-md">
        <div className="flex items-center">
          <span className="py-2 flex items-center gap-2">
            <svg
              width="24"
