...
interface PengirimanProps {
  onShippingChange: (cost: number, name: string) => void // Kirim nama juga
}

const Pengiriman: React.FC<PengirimanProps> = ({ onShippingChange }) => {
  ...
  const shippingOptions = [
    {
      ...
      cost: 0,
      originalCost: 14000,
      estimation: 'Estimasi tiba 25-27 Sep',
    },
    {
      id: 2,
      name: 'Standar',
      cost: 8000,
      originalCost: 17000,
      estimation: 'Estimasi tiba 26-28 Sep',
    },
    {
      id: 3,
      name: 'Instant',
      cost: 20000,
      originalCost: 25000,
      estimation: 'Estimasi tiba 25 Sep (Hari ini)',
    },
    {
      id: 4,
      name: 'Cargo',
      cost: 15000,
      originalCost: 20000,
      estimation: 'Estimasi tiba 28-30 Sep',
    },
  ]
          ...
          <div className="bg-white p-4 rounded-t-[24px] w-full max-w-md shadow-lg z-30 animate-slide-up font-nunito relative">
              ...
              {shippingOptions.map((option) => (
                <div
                  ...
                  className={`border p-2 cursor-pointer ${
                    selectedOption === option.id ? 'border-emerald-500' : ''
                  }`}
                ...
                >
                  <div className="flex justify-between items-center">
                    ...
                    <div className="text-right">
                      {/* Harga Asli Dicoret */}
                      {option.cost !== option.originalCost && (
                        <span className="font-nunito text-gray-500 text-[13px] mr-1">
                          <s>Rp {option.originalCost}</s>
                        </span>
                      )}
                      <span className="text-emerald-500 font-nunito text-[13px] font-bold">
                        {option.cost === 0 ? 'Gratis' : `Rp ${option.cost}`}
                      </span>
                    </div>
                  </div>

                  {/* Estimasi Pengiriman */}
                  <p className="text-[11px] text-black">{option.estimation}</p>
                </div>
              ))}
            </div>
            ...
            <div className="mt-12 px-4">
              <button
                ...
                onClick={() => setShowPopup(false)} // Tutup popup saat diklik
              >
                Konfirmasi
              </button>
            </div>
